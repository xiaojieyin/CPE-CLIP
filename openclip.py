...
def evaluate(val_loader, model, text_features, args):
    ...
    for i, (images, target) in enumerate(val_loader):
        ...
        image_weights, text_weights = get_entropy_weight(output * 100.0)
...
def main_worker(args):
    ...
    model, _, preprocess = open_clip.create_model_and_transforms(
        'ViT-B-16',
        pretrained='clip_ckpt/clip-vit-b-16-laion2b-s34b-b88k/open_clip_pytorch_model.bin'
    )
...
def evaluate_synonym(val_loader, model, text_features, mask, args):
    ...
    save_dir = f"./features/pre_extracted_feat/openclip/seed{args.seed}"
    ...
    for i, (image_features, target) in enumerate(tqdm(pre_features, total=len(pre_features))):
        ...
        image_weights, text_weights = get_entropy_weight_(output * 100.0, mask)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='AWT for OpenClip')
