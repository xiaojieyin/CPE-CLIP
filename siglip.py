...
def optimal_transport(logits, image_weights, text_weights):
    ...
    image_weights = image_weights.to(logits.device)
    text_weights = text_weights.to(logits.device)
...
MODEL_PATH = 'clip_ckpt/siglip-base-patch16-224'
...
def evaluate_synonym(val_loader, model, text_features, mask, args):
    ...
    save_dir = f"./features/pre_extracted_feat/siglip/seed{args.seed}"
    os.makedirs(save_dir, exist_ok=True)

    if not os.path.isfile(os.path.join(save_dir, f"{args.test_set}.pth")):
    ...
    for i, (image_features, target) in enumerate(tqdm(pre_features, total=len(pre_features))):
        ...
        image_weights, text_weights = get_entropy_weight(output * model.logit_scale.exp() + model.logit_bias, mask)
...
def main_worker_synonym(args):
    ...
    padding_text_features = torch.zeros((n_cls, n_prompt, n_dim), device=device).float()
