...
from data import imagnet_prompts
...
def get_cls_descs_single_question(args, classname, question):
    assert args.llm in ["gpt-3.5-turbo", "gpt-4o", "qwen-plus", "gpt-4-turbo"]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prompt_folder', type=str, default='utils',
                        help='The root folder to save all generated prompts.')
    parser.add_argument('--question_file', type=str, default='0_Dataset_Questions.yaml',
                        help='The file name of generated Step-1 dataset questions.')
    parser.add_argument('--desc_count', type=int, default=50,
                        help='Minimum number of descriptions for each class to generate.')
    ...
    for dataset_name, questions in dataset_questions.items():
        if dataset_name not in ['oxford_pets']:
            continue
        print(f"{dataset_name}: {len(get_classnames(dataset_name))} classes to generate.")

        # get synonyms
        synonym_file = os.path.join('descriptions/synonyms', f'{dataset_name}.json')
        synonyms = json.load(open(synonym_file))

        data = {}

        for i, classname in enumerate(get_classnames(dataset_name)):
            ...
            while len(descriptions) < args.desc_count:
                try:
                    ...
                    gen_questions = iter(questions)
                print(
                    f"    Generating for {dataset_name}'s [{i}] {classname}. {len(descriptions)} descriptions generated.")
                descriptions = list(filter(lambda x: x != "" and x != " . ", descriptions))
