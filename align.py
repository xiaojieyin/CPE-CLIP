...
MODEL_PATH = 'clip_ckpt/align-base'
...
def evaluate_synonym(val_loader, model, text_features, mask, args):
    ...
    save_dir = f"./features/pre_extracted_feat/align/seed{args.seed}"
    ...
    for i, (image_features, target) in enumerate(tqdm(pre_features, total=len(pre_features))):
        ...
        image_weights, text_weights = get_entropy_weight(output / model.temperature, mask)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='AWT for ALIGN')
    ...
    for set_id in datasets:
        ...
        results[set_id] = main_worker_raw(args)
